        ...
        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 30px;">
          <div style="flex: 1;">
            <div style="margin-bottom: 15px;">
              <strong style="color: #1B2735;">ID de Compra:</strong> ${purchaseData.saleId}
            </div>
            <div style="margin-bottom: 15px;">
              <strong style="color: #1B2735;">Fecha del Evento:</strong> ${new Date(purchaseData.eventoFecha).toLocaleDateString('es-CL')}
            </div>
            <div style="margin-bottom: 15px;">
              <strong style="color: #1B2735;">Total Pagado:</strong> $${purchaseData.total.toLocaleString('es-CL')}
            </div>
            <div style="margin-bottom: 15px;">
              <strong style="color: #1B2735;">Fecha de Compra:</strong> ${new Date(purchaseData.timestamp).toLocaleDateString('es-CL')} ${new Date(purchaseData.timestamp).toLocaleTimeString('es-CL')}
            </div>
          </div>
        ...
        <div style="margin-bottom: 30px;">
          ...
          ${purchaseData.tickets.map(ticket => `
            <div style="display: flex; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid #eee;">
              ...
              <span>$${(ticket.precio * ticket.cantidad).toLocaleString('es-CL')}</span>
            </div>
          `).join('')}
        </div>
          ...
          <div style="margin-bottom: 30px;">
            ...
            ${purchaseData.attendees.map((attendee, index) => `
              <div style="padding: 5px 0;">
                <strong>${index + 1}.</strong> ${attendee.nombre} (${attendee.tipo})
              </div>
            `).join('')}
          </div>
        ` : ''}
      ...
      <main className={styles.mainContent}>
        ...
        >
          <span className={styles.verificationText}>
            {isGeneratingPDF ? 'Generando PDF...' : 'De verificación'}
          </span>
        </div>
